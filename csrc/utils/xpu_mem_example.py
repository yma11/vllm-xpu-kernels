...
def example_usage():
    ...
    def python_malloc(allocation_handle):
        """Callback when memory is allocated. Receives (device, size, ptr, handle)."""
        device, size, ptr, handle = allocation_handle
        print(f"[Python] Allocating {size} bytes on device {device}, ptr=0x{ptr:x}")
        allocation_map[ptr] = (device, size, handle)
        return None
    ...
    def python_free(ptr):
        ...
        if ptr in allocation_map:
            device, size, handle = allocation_map.pop(ptr)
    ...
    device, size, handle = allocation_map[ptr]
    print(
        f"allocation succeeds: ptr=0x{ptr:x}, device={device}, size={size}, handle={handle}"
    )

    xpumem_allocator.python_unmap_and_release(device, size, ptr, handle)

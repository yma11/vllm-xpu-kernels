...
// NOTE: these are borrowed references.
...
void* my_malloc(ssize_t size, int device, sycl::queue* queue) {
  ...
  // Keep CUDA cumem allocator parity: pass a single handle tuple.
...
static PyObject* python_create_and_allocate(PyObject* self, PyObject* args) {
  ...
  // Wake path: remap physical memory to the same reserved virtual address.
  (void)recv_device;
  (void)recv_size;
  (void)recv_handle;
